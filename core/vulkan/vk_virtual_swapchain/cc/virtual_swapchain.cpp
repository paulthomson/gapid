...
extern "C" void virtual_swapchain_write_png(stbi_write_func* func, void* context, uint8_t* image_data, size_t size, uint32_t width, uint32_t height, uint32_t image_format) {
  WritePng(func, context, image_data, size, width, height, static_cast<VkFormat>(image_format));
}
